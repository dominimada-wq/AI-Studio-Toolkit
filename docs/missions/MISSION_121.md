...
> **MISSION PRÉPARÉE, NON IMPLÉMENTÉE.** Ce document fixe le périmètre autorisé par l'architecte après le micro-audit dédié du contrat dtype/optimizer/mémoire réel d'OneTrainer (variante A retenue). Aucun code n'est encore écrit. Implémentation à ouvrir uniquement après validation explicite de ce document.
...
Suite complète exécutée, nombre exact confirmé avant tout commit, comme pour toute mission.

## 9. Smoke réel (prévu, non exécuté dans ce document)

À proposer une fois l'implémentation et la suite complète validées, jamais exécuté automatiquement. Scénario : réutiliser le cas SD1.5 déjà validé (Missions 097-101, 120), en configurant explicitement un sous-ensemble du vocabulaire UI (section 3.2) pour `train_dtype`/`unet_weight_dtype`/`text_encoder_weight_dtype` — valeurs choisies au moment du smoke, jamais présumées ici comme un nouveau défaut. Chemin à prouver : `Training` → `OneTrainerSettings` → configuration préparée → snapshot `TrainingJob` immuable → lancement réel via `TrainingJobRunner` → vrai step GPU → `lora.safetensors` réel produit. Aucun changement d'environnement CUDA/Torch/OneTrainer/drivers. Un éventuel benchmark VRAM comparatif est explicitement hors périmètre de la preuve requise par cette mission — pourra être décidé séparément.
