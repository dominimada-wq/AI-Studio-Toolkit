...
> **MISSION CLÔTURÉE — commit, tag et GitHub Release publiés.** `ComfyUIEngine._request_json()` (`src/engines/comfyui_engine.py:687-707`) capturait `urllib.error.HTTPError`, lisait son corps, puis passait ce corps dans le même `json.loads()` que le chemin HTTP réussi — si ce corps était un JSON valide, il était retourné à l'appelant comme s'il provenait d'une réponse 2xx, et le code HTTP réel (`error.code`) n'était jamais lu nulle part dans cette méthode. Le symptôme le plus visible était dans `wait_for_result()` : une `HTTPError` avec un corps JSON parseable pendant le polling `/history/{prompt_id}` pouvait être interprétée comme « résultat pas encore disponible » et finir masquée derrière le timeout applicatif générique (~120s). M151 centralise dans `_request_json()` un invariant simple : toute réponse HTTP non-2xx produit désormais immédiatement une `ComfyUIEngineError`, jamais un JSON métier normal. Voir §12 pour le scope exact, §5 pour le contrat source confirmé du body `/prompt`, §23 pour les résultats réels de l'implémentation et §24 pour la clôture Git.
...
Commit fonctionnel `b750fe1677eb242b4012fdd371e088b542da267b` (« Fail fast on ComfyUI HTTP errors », 3 fichiers : `src/engines/comfyui_engine.py`, `tests/integration/test_comfyui_engine.py`, `docs/missions/MISSION_151.md`, 433 insertions(+), 1 suppression(-)), poussé sur `main` dès le premier essai (`a75b0a3..b750fe1`). Tag annoté `v0.2-mission151` créé exactement sur ce commit (objet tag local et distant `b8af209afed7d07d130358f5e922e64f09d92dc8`, peeled target local et distant tous deux `b750fe1677eb242b4012fdd371e088b542da267b`, vérifiés identiques), poussé et confirmé sur `origin`. `v0.2-mission150` (objet `1372e6b78585d3838b453f846f1df3741a3e51a4`, peeled `e36d3b075683547ef6aa0d8a07cc3c791be8fb24`), `v0.2-mission149` (peeled `98d9c3c5057edfd67757ac4d0011fb550d66ed72`), `v0.2-mission148` (peeled `dffe92c09dbcd55e5c950d8c065708682034e0fe`), `v0.2-mission147` (peeled `3f9d9e7a8e02b05a72f24bfae2e233e4b62ca06a`) et `v0.2-mission146` (peeled `24613861960e3f4e6c87c9a80c8f111f00f74b7a`) reconfirmés inchangés. GitHub Release `v0.2-mission151` publiée manuellement (titre « v0.2-mission151 — Fail Fast on ComfyUI HTTP Errors »). Le présent commit documentaire de régularisation post-Release (`CHANGELOG.md`, `docs/PROJECT_CONTEXT.md`, `docs/missions/MISSION_151.md`) n'appartient pas au tag `v0.2-mission151` — celui-ci reste durablement sur le commit fonctionnel ci-dessus, jamais déplacé. Aucun élargissement de scope, aucune fonctionnalité ajoutée après validation. Le finding distinct découvert par le mini-audit préalable à Mission 150 (`_request_json()` absorbant une `HTTPError` JSON pendant `/history`) est celui que cette mission résout ; les autres dettes de l'audit global post-Mission 150 (`docs/PROJECT_CONTEXT.md`, section « Problèmes connus / dettes ») restent explicitement hors périmètre et ouvertes.
