...
> **MISSION CLÔTURÉE — commit, tag et GitHub Release publiés.** `ComfyUIEngine.wait_for_result()` (`src/engines/comfyui_engine.py`) détectait le succès uniquement via la présence d'une image exploitable dans `outputs` — il ignorait totalement le statut terminal officiel que ComfyUI fournit déjà dans le même appel `/history/{prompt_id}` (`status.status_str`). Un crash réel de node ComfyUI était donc indiscernable de « pas encore prêt » et le Toolkit continuait de poller jusqu'au timeout applicatif générique (~120s), masquant l'erreur réelle derrière un message de timeout non informatif. M150 ajoute une détection positive, minimale et rétrocompatible de cet échec terminal, sans toucher au contrat de succès historique ni à aucun fichier hors de `src/engines/comfyui_engine.py`. Voir §12 pour le scope exact, §3 pour la démonstration du contrat ComfyUI réel confirmé par lecture directe du code source officiel, §22 pour les résultats réels de l'implémentation et §23 pour la clôture Git.
...
**Rappel explicite** : M150 ne corrige pas et ne prétend pas corriger le finding séparé sur `_request_json()`/`HTTPError` pendant `/history`.

## 23. Clôture Git

Commit fonctionnel `e36d3b075683547ef6aa0d8a07cc3c791be8fb24` (« Fail fast on ComfyUI execution errors », 3 fichiers : `src/engines/comfyui_engine.py`, `tests/integration/test_comfyui_engine.py`, `docs/missions/MISSION_150.md`, 558 insertions(+), 3 suppressions(-)), poussé sur `main` (`a8fe626..e36d3b0` — le premier essai de push a rencontré une erreur serveur GitHub transitoire (`Internal Server Error`), sans rapport avec une divergence/un commit étranger, diagnostiquée puis résolue par un simple nouvel essai identique, réussi). Tag annoté `v0.2-mission150` créé exactement sur ce commit (objet tag local et distant `1372e6b78585d3838b453f846f1df3741a3e51a4`, peeled target local et distant tous deux `e36d3b075683547ef6aa0d8a07cc3c791be8fb24`, vérifiés identiques), poussé et confirmé sur `origin`. `v0.2-mission149` (peeled `98d9c3c5057edfd67757ac4d0011fb550d66ed72`), `v0.2-mission148` (peeled `dffe92c09dbcd55e5c950d8c065708682034e0fe`), `v0.2-mission147` (peeled `3f9d9e7a8e02b05a72f24bfae2e233e4b62ca06a`), `v0.2-mission146` (peeled `24613861960e3f4e6c87c9a80c8f111f00f74b7a`) et `v0.2-mission145` (peeled `3dcc625f02bca4e9398c1db8c933550058513582`) reconfirmés inchangés. GitHub Release `v0.2-mission150` publiée manuellement (titre « v0.2-mission150 — Fail Fast on ComfyUI Terminal Execution Errors »). Le présent commit documentaire de régularisation post-Release (`CHANGELOG.md`, `docs/PROJECT_CONTEXT.md`, `docs/missions/MISSION_150.md`) n'appartient pas au tag `v0.2-mission150` — celui-ci reste durablement sur le commit fonctionnel ci-dessus, jamais déplacé. Aucun élargissement de scope, aucune fonctionnalité ajoutée après validation ; le finding distinct `_request_json()`/`HTTPError` pendant `/history` reste explicitement hors périmètre et ouvert (§17).
