...
**Nouveaux tests à écrire** : `list_loras()` (succès, réponse `LoraLoader` absente/malformée, échec réseau/URL invalide — mirroir exact de `list_checkpoints()`) ; builders (`LoraLoader` inséré quand `lora_name` fourni, absent et graphe byte-for-byte identique sinon) ; `GenerationManager.generate()` (forwarding correct, comportement inchangé sans LoRA) ; roundtrip `ApplicationSettings` (nouveaux champs, compatibilité JSON antérieur sans ces clés) ; `SettingsPage` (combobox peuplé, sauvegarde, persistance après réouverture).
...
- Clôture Git (commit/tag/Release) : **en cours** — voir section 16.
