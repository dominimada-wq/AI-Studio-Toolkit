...
class MainWindowCloseEventComfyUILifecycleGuardTest(unittest.TestCase):
    """
    Mission 114: comfyui_lifecycle_manager.confirm_safe_to_close() is
    consulted right after confirm_no_active_training() and before the
    four dirty-draft guards -- same orchestration-only testing
    philosophy as MainWindowCloseEventOrchestrationTest above (the
    manager's own state machine is already covered in full by
    test_comfyui_lifecycle_manager.py; this class only proves
    ...
    def tearDown(self):
        # Defensive reset, unconditional: several tests below fabricate
        # comfyui_lifecycle_manager._state directly (STARTING/STOPPING/
        # RUNNING_OWNED) to isolate one scenario, always inside their own
        # patched-QMessageBox `with` block -- but this real close() runs
        # after that block has already exited and the patch reverted. A
        # test that forgot (or a future one) to restore STOPPED before
        # returning would otherwise hit a real, actually-blocking
        # QMessageBox here -- exactly the bug found in this class after
        # implementation (test_real_starting_blocks_close_with_a_warning
        # left _state == STARTING behind it). Resetting here, always,
        # makes this teardown safe regardless of what any test body did.
        from src.ui.comfyui_lifecycle_manager import STOPPED
    ...
    def test_comfyui_guard_false_ignores_close_before_any_dirty_guard_runs(self):
        with patch.object(
            self.window.comfyui_lifecycle_manager, "confirm_safe_to_close", return_value=False
        ) as guard_mock, \
                patch.object(self.window.prompts_page, "confirm_context_change") as prompts_mock, \
                patch.object(self.window.inference_page, "shutdown") as shutdown_mock:
            event = QCloseEvent()
    ...
    def test_comfyui_guard_true_lets_close_proceed(self):
        patchers = self._patch_other_guards()
        with patchers[0], patchers[1], patchers[2], patchers[3], patchers[4], patchers[5], patchers[6], \
                patch.object(
                    self.window.comfyui_lifecycle_manager, "confirm_safe_to_close", return_value=True
                ) as guard_mock, \
                patch.object(self.window.inference_page, "shutdown") as shutdown_mock:
            event = QCloseEvent()
            self.window.closeEvent(event)
    ...
    def test_guard_runs_after_training_and_before_prompts(self):
        order = []
        with patch.object(
                    self.window.training_page, "confirm_no_active_training",
                    side_effect=lambda message: order.append("training") or True,
                ), \
                patch.object(
        ...
        self.assertEqual(order, ["training", "comfyui_lifecycle", "prompts"])

    def test_real_external_active_closes_without_any_dialog_or_stop(self):
        from src.ui.comfyui_lifecycle_manager import EXTERNAL_ACTIVE

        self.window.comfyui_lifecycle_manager._state = EXTERNAL_ACTIVE
        patchers = self._patch_other_guards()
        with patchers[0], patchers[1], patchers[2], patchers[3], patchers[4], patchers[5], patchers[6], \
                patch("src.ui.comfyui_lifecycle_manager.QMessageBox") as box, \
    ...
    def test_real_starting_blocks_close_with_a_warning(self):
        from src.ui.comfyui_lifecycle_manager import STARTING

        self.window.comfyui_lifecycle_manager._state = STARTING
    ...
    def test_real_running_owned_no_leaves_comfyui_running_and_closes(self):
        ...
        with patchers[0], patchers[1], patchers[2], patchers[3], patchers[4], patchers[5], patchers[6], \
                patch("src.ui.comfyui_lifecycle_manager.QMessageBox") as box, \
                patch.object(self.window.comfyui_lifecycle_manager, "stop") as stop_mock, \
                patch.object(self.window.inference_page, "shutdown") as shutdown_mock:
        ...
        shutdown_mock.assert_called_once()
        # No ownership persisted anywhere -- state is simply left as-is.
        self.assertEqual(self.window.comfyui_lifecycle_manager.state, RUNNING_OWNED)
        # tearDown() itself now resets this defensively before its real
        # close() -- this explicit reset is kept anyway as documentation
        # of intent and defense in depth, not the only thing preventing
        # a real dialog here.
        self.window.comfyui_lifecycle_manager._state = STOPPED
    ...
    def test_real_running_owned_yes_defers_close_until_stop_actually_finishes(self):
        ...
        def _fake_stop():
            # Simulates the real Stop sequence finishing later -- see
            # test_comfyui_lifecycle_manager.py for the exhaustive
            # coverage of the real terminate()/kill() escalation itself.
            self.window.comfyui_lifecycle_manager._state = STOPPED
            self.window.comfyui_lifecycle_manager._resume_close_if_pending()

        with patchers[0], patchers[1], patchers[2], patchers[3], patchers[4], patchers[5], patchers[6], \
                patch("src.ui.comfyui_lifecycle_manager.QMessageBox") as box, \
                patch.object(self.window.comfyui_lifecycle_manager, "stop", side_effect=_fake_stop), \
                patch.object(self.window.inference_page, "shutdown") as shutdown_mock:
            box.Yes, box.No = RealQMessageBox.Yes, RealQMessageBox.No
            ...
            # The deferred window.close() call inside _resume_close_if_pending()
            # re-entered closeEvent() synchronously -- confirm_safe_to_close()
            # is now STOPPED, so no second confirmation dialog, and the
            # window really did close this time.
            shutdown_mock.assert_called_once()
