...
- **Mission 152 — Harden LoRA Exposure Root Inspection**
  - [Résumé (Mission 152)](#résumé-mission-152)
  - [Tests ajoutés (Mission 152)](#tests-ajoutés-mission-152)
  - [État du projet (Mission 152)](#état-du-projet-mission-152)
- **Mission 151 — Fail Fast on ComfyUI HTTP Errors**
  - [Résumé (Mission 151)](#résumé-mission-151)
  - [Tests ajoutés (Mission 151)](#tests-ajoutés-mission-151)
  - [État du projet (Mission 151)](#état-du-projet-mission-151)
...
- **Mission 149 — Guard LoRA Exposure Root Changes While Exposed**
  ...
  - [État du projet (Mission 149)](#état-du-projet-mission-149)
...
- **Mission 145 — Guard Training Deletion Against Active Jobs and Filesystem Orphaning**
  ...
  - [État du projet (Mission 145)](#état-du-projet-mission-145)
- **Mission 144 — Prevent Silent Fallback on Corrupt Settings and LoRA Library Storage**
  - [Résumé (Mission 144)](#résumé-mission-144)
  - [Tests ajoutés (Mission 144)](#tests-ajoutés-mission-144)
  - [État du projet (Mission 144)](#état-du-projet-mission-144)
...
- **Mission 142 — Guard ComfyUI Teardown Against Stale Terminate Timers**
  ...
  - [État du projet (Mission 142)](#état-du-projet-mission-142)
...
- **Mission 139 — Isolate Materialized Training Concept Per Job**
  ...
  - [État du projet (Mission 139)](#état-du-projet-mission-139)
...
- **Mission 087 — Central LoRA Library Foundation (Registry + Configurable Path)**
  ...
  - [État du projet (Mission 087)](#état-du-projet-mission-087)
...
## v0.2-mission152 — 2026-09-25

*Note de régularisation* : cette entrée est rédigée pendant la régularisation documentaire post-publication de Mission 152 — commit fonctionnel, tag et Release sont déjà tous réels au moment de la rédaction.

### Résumé (Mission 152)

Issue d'un mini-audit READ-ONLY ciblé mené après clôture complète de Mission 151, sur la robustesse filesystem du verrou d'exposition LoRA introduit par Mission 149, validé par l'architecte, vérifié par lecture directe du code source CPython `pathlib.py` de la version réellement utilisée par ce projet et par expérimentation locale reproduisant chaque scénario (ACL Windows réelles, drive letter inexistante, suppression concurrente). Root cause : `LoRALibraryManager.has_any_exposure(expose_root)` (`src/managers/lora_library_manager.py`) délégait entièrement à `_find_existing_alias()`, qui teste `Path.is_dir()` puis `Path.glob()` — deux primitives `pathlib` qui avalent silencieusement plusieurs classes d'erreurs filesystem distinctes (permission refusée, média non prêt, erreur réseau) en les réduisant toutes à `False`/liste vide, exactement comme un root réellement absent. Séquence de bug reproductible : un root Forge/ComfyUI exposé devient temporairement inaccessible → `has_any_exposure()` retourne `False` par erreur → le verrou Mission 149 ne se déclenche pas → l'utilisateur change le root en Settings → un nouvel hardlink est créé dans le nouveau root → une suppression future du LoRA ne désexpose que le root **actuellement configuré** → l'ancien hardlink, orphelin, survit indéfiniment sur disque — exactement la classe de bug que Mission 149 avait pour objectif de fermer.

Contrat retenu : un contrat sémantique à trois états, obligatoire, implémentation libre. État 1 — root réellement absent (`FileNotFoundError`/`NotADirectoryError`) : `False`, comportement M149 inchangé. État 2 — root inspectable : `True`/`False` selon une exposition réelle, comportement M149 inchangé. État 3 — inspection non concluante (`PermissionError`, volume non prêt, toute autre `OSError`) : ne jamais retourner `False` silencieusement — nouvelle exception métier dédiée `LoRAExposureRootInspectionError`, fail-safe/fail-closed pour ce changement de Setting précis. Exception définie dans `src/managers/lora_library_manager.py` (jamais dans `application_settings_manager.py`, pour éviter un import circulaire — celui-ci importe déjà `LoRALibraryManager` depuis ce module), propagée non catchée à travers `ApplicationSettingsManager.update()` exactement comme l'ambiguïté multi-alias préexistante de `LoRALibraryError`. `has_any_exposure()` ne délègue plus à `_find_existing_alias()` — nouvelle primitive statique dédiée `_list_expose_subfolder()`, un seul `os.scandir()` par appel, motif `*__{lora_id}.*` reproduit via `fnmatch`. Implémentation retenue : distinction par type d'exception uniquement (`FileNotFoundError`/`NotADirectoryError` = absence, tout autre `OSError` = inconclusif), plus petite et déjà portable par construction que l'approche `errno`/`winerror` anticipée par le draft de mission. `_find_existing_alias()`/`_expose()`/`_unexpose()`/`delete()` (Mission 095/135) confirmés octet pour octet inchangés. `SettingsPage.save_application_settings()` capte désormais la nouvelle exception dans le même tuple `except` que `LoRAExposureRootLockedError` — dialogue `QMessageBox.critical()` + resynchronisation UI identiques, aucune persistance en cas d'échec, Forge et ComfyUI restent verrouillés indépendamment.

### Tests ajoutés (Mission 152)

**+14 tests nets** (2877 → 2891 tests collectés). 10 nouveaux dans `LoRALibraryManagerHasAnyExposureTest`/`ApplicationSettingsExposureRootLockTest` (`tests/integration/test_lora_library_roundtrip.py`) : bibliothèque vide ne touchant jamais le filesystem, `PermissionError`/`OSError` générique/`NOT_READY` simulé levant tous l'exception dédiée, message d'exception sans trace de traceback, inspection non concluante Forge/ComfyUI propagée sans mutation, indépendance stricte des deux providers, no-op préservé même si l'inspection échouerait, `update()` combiné refusé ne persistant aucun champ. 4 nouveaux dans `SettingsPageSaveErrorTest` (`tests/integration/test_settings_page.py`) : dialogue affiché sans lever, resynchronisation du champ sur la valeur réellement persistée, aucune persistance supplémentaire, page réutilisable pour une sauvegarde réelle après l'échec simulé.

### État du projet (Mission 152)

**2891 tests collectés.** Ciblés : `LoRALibraryManagerHasAnyExposureTest` + `ApplicationSettingsExposureRootLockTest` **30/30** (20 tests M149 préexistants + 10 nouveaux, tous verts), `test_lora_library_roundtrip.py` complet **127/127**, `test_settings_page.py` complet **94/94** (90 préexistants + 4 nouveaux). Non-régression : `test_lora_roundtrip.py` (Workbench LoRA) **275/275**, les 20 tests Mission 149 et les tests Mission 135 pertinents rejoués sans aucune modification de leur code. **Suite complète : 2891 collectés/2891 passés/0 échoué.** Équation : 2877 (clôture Mission 151) + 14 nets ajoutés par Mission 152 = **2891**, cohérent. `git diff --check` clean. Exactement les 6 fichiers annoncés modifiés (`src/managers/lora_library_manager.py`, `src/managers/application_settings_manager.py`, `src/ui/pages/settings_page.py`, `tests/integration/test_lora_library_roundtrip.py`, `tests/integration/test_settings_page.py`, `docs/missions/MISSION_152.md`) — aucun fichier étranger. Aucun smoke manuel requis, justifié : mécanisme entièrement démontré par tests déterministes (`os.scandir` simulé), aucune UI nouvelle, chaîne de propagation vers `SettingsPage` déjà confirmée compatible par le mini-audit préalable. Commit fonctionnel `0148986472bc9d18d3c1bb3f7645c2c7d15531a1` (« Harden LoRA exposure root inspection »), tag `v0.2-mission152` (objet `dd6b4a3c34b755c9acab68e181414668917028e0`), GitHub Release publiée manuellement. Dette « faux négatif filesystem du verrou d'exposition LoRA (Mission 149) », identifiée par le mini-audit préalable et conservée ouverte depuis, est désormais **résolue par Mission 152**. Non-goals explicitement confirmés, non résolus par cette mission : aucune persistance des chemins d'exposition, aucun registre des anciens roots, aucune migration/récupération automatique des hardlinks déjà orphelins avant cette mission — mission strictement prospective, comme M149 l'était déjà. Hors périmètre, confirmé et non traité : suppression d'un LoRA Central Library pendant une génération active non gardée, `ApplicationSettingsStorageError` pouvant laisser l'UI visuellement non resynchronisée, processus OneTrainer potentiellement orphelin après fermeture anormale, absence de liveness PID/réconciliation avancée des Jobs `unknown`, limites per-job delete/retry, timeout Forge fixe/non configurable, limitation de l'API synchrone Forge, flakiness Forge des tests réel-process (TEST DEBT), Workspace first-save rollback gap, path-ID traversal defense-in-depth, caption sidecar illisible provoquant silencieusement le fallback `trigger_word`, `create_job()` partial cleanup, Resume/process identity, cascade de suppression Character (future), `rolling_backup` OneTrainer (NON-ISSUE), New/Open Project vs génération Inference active (Mission 085, NON-ISSUE reconfirmé), et tout autre candidat de l'audit global post-Mission 150 non encore traité.

---

## v0.2-mission151 — 2026-09-25

*Note de régularisation* : cette entrée est rédigée pendant la régularisation documentaire post-publication de Mission 151 — commit fonctionnel, tag et Release sont déjà tous réels au moment de la rédaction.

### Résumé (Mission 151)

Issue du Candidate 1 de l'audit global READ-ONLY mené après clôture complète de Mission 150, validé par l'architecte, puis verrouillé par un mini-audit architectural/fonctionnel READ-ONLY dédié au contrat complet de `_request_json()` et de ses six call sites, confirmant par lecture directe du code source officiel `comfyanonymous/ComfyUI` (`execution.py`/`server.py`, branche `main` — pas inféré) le contrat réel de `/prompt` (échec de validation → HTTP 400, corps `{"error": {"type", "message", "details", "extra_info"}, "node_errors": {...}}`) et de `/history/{prompt_id}` (toujours HTTP 200, y compris pour un `prompt_id` inconnu — une `HTTPError` réelle sur cette route ne peut provenir que d'un crash serveur ou d'une panne réseau/proxy, jamais d'un `prompt_id` inconnu). Root cause : `ComfyUIEngine._request_json()` (`src/engines/comfyui_engine.py`) capturait `urllib.error.HTTPError`, lisait son corps, puis laissait ce corps rejoindre le même chemin `json.loads()`-puis-`return` qu'une réponse 2xx réussie — un statut HTTP non-2xx pouvait donc être silencieusement traité comme une donnée JSON métier normale, `error.code` étant entièrement ignoré. Le déclencheur le plus coûteux se situait dans `wait_for_result()` : une `HTTPError` au corps JSON parseable survenant pendant le polling `/history/{prompt_id}` pouvait être lue comme « pas encore prêt » et continuer d'être pollée jusqu'au timeout applicatif générique (~120s), masquant l'échec réel derrière un message de timeout non informatif.

Contrat retenu : toute `HTTPError` lève désormais immédiatement une `ComfyUIEngineError` — son corps ne peut plus jamais être retourné comme une réponse JSON normale. Le message inclut toujours le code HTTP et la raison (`error.code`/`error.reason`) quand disponibles. Extraction défensive limitée d'un détail textuel additionnel, par priorité stricte et sans recherche récursive : (1) `data["error"]["message"]` si `error` est un dict avec un `message` textuel non vide (forme réelle confirmée de l'échec de validation `/prompt`), (2) sinon `data["message"]` au premier niveau si textuel non vide, (3) sinon aucun détail — repli sur le seul code+raison. `node_errors`, `extra_info`, `details`, tout traceback, et toute structure JSON brute ne sont jamais exposés dans le message. Un corps non-JSON ou vide continue de lever immédiatement avec le code HTTP, sans jamais provoquer d'exception secondaire. Aucun retry n'est introduit — chaque `HTTPError` échoue immédiatement, une seule fois. Le chemin HTTP 2xx (`json.loads()`/`except JSONDecodeError`) et le traitement `URLError`/`OSError`/timeout restent intégralement inchangés — diff borné à un seul hunk dans `except HTTPError`. Aucun des six call sites de `_request_json()` (`submit()`, `wait_for_result()`, `upload_image()`, `list_checkpoints()`, `list_loras()`, `_list_ksampler_combo_values()`) n'a nécessité de modification — cinq d'entre eux levaient déjà accidentellement une `ComfyUIEngineError` sur `HTTPError` via une clé métier manquante ; seul le message change, jamais le comportement observable. `download_output()` confirmé ne pas être un call site de `_request_json()` — sa propre gestion inline de `HTTPError` reste inchangée.

**Séparation M150/M151** : le mécanisme de Mission 150 (`_comfyui_execution_error_message()`, prédicat `status.status_str == "error"` sur une réponse HTTP 2xx) et celui de Mission 151 (levée immédiate sur tout statut HTTP non-2xx) couvrent des chemins de code mutuellement exclusifs dans le modèle d'exception d'`urllib` — reconfirmé par un diff ne touchant aucune ligne du mécanisme M150 et par les 15 tests M150 repassant inchangés.

### Tests ajoutés (Mission 151)

**+7 tests nets** (2870 → 2877 tests collectés). 6 nouveaux dans `ComfyUIEngineSubmitTest` (`tests/integration/test_comfyui_engine.py`) : message incluant code HTTP + détail `error.message` de `/prompt` (avec marqueurs uniques prouvant l'absence de fuite d'autres champs), repli sur un `message` au premier niveau, repli sur le seul code+raison en l'absence de champ connu, corps JSON invalide levant tout de même avec le code HTTP, corps vide levant tout de même avec le code HTTP, message incluant la raison HTTP quand exploitable. 1 nouveau dans `ComfyUIEngineWaitForResultTest` : fail-fast démontré par `mock_urlopen.assert_called_once()`/`mock_sleep.assert_not_called()` sur une `HTTPError` survenant pendant le polling `/history`.

### État du projet (Mission 151)

**2877 tests collectés.** Ciblés : Submit + WaitForResult **27/27**, `test_comfyui_engine.py` complet **117/117**. Non-régression : `test_generation_manager.py` + `test_generation_worker.py` **91/91**, `test_inference_page.py` (widgets Qt réels) **230/230** (le traceback `QLabel.setText(MagicMock)` observé est préexistant, bénin, sans rapport avec M151). **Suite complète : 2877 collectés/2877 passés/0 échoué.** Équation : 2870 (clôture Mission 150) + 7 nets ajoutés par Mission 151 = **2877**, cohérent. `git diff --check` clean. Exactement les 3 fichiers annoncés modifiés (`src/engines/comfyui_engine.py`, `tests/integration/test_comfyui_engine.py`, `docs/missions/MISSION_151.md`) — aucun fichier étranger. Aucun smoke manuel requis, justifié : mécanisme entièrement démontré par tests déterministes (`urllib.request.urlopen` simulé, call-count), aucune UI nouvelle, chaîne de propagation vers `InferencePage` déjà confirmée compatible sans modification par le mini-audit préalable. Commit fonctionnel `b750fe1677eb242b4012fdd371e088b542da267b` (« Fail fast on ComfyUI HTTP errors »), tag `v0.2-mission151` (objet `b8af209afed7d07d130358f5e922e64f09d92dc8`), GitHub Release publiée manuellement. Dette « HTTPError absorbée par `_request_json()` », identifiée comme finding distinct par le mini-audit préalable à Mission 150 et conservée ouverte depuis, est désormais **résolue par Mission 151**. Hors périmètre, confirmé et non traité : processus OneTrainer potentiellement orphelin après fermeture anormale, absence de liveness PID/réconciliation avancée des Jobs `unknown`, limites per-job delete/retry, timeout Forge fixe/non configurable et message potentiellement trompeur dans certains cas, limitation de l'API synchrone Forge, flakiness Forge des tests réel-process (TEST DEBT), New/Open Project ne protégeant pas encore une génération Inference active, suppression d'un LoRA Central Library pendant une génération active non gardée, erreurs filesystem/`PermissionError`/TOCTOU possibles dans `has_any_exposure()`, alias historiques d'anciens expose roots non découvrables automatiquement, `ApplicationSettingsStorageError` pouvant laisser l'UI visuellement non resynchronisée, Workspace first-save rollback gap dans `create_workspace_with_default_character()`, path-ID traversal defense-in-depth, caption sidecar illisible provoquant silencieusement le fallback `trigger_word`, `create_job()` partial cleanup, Resume/process identity, cascade de suppression Character (future, ne devant jamais concerner la Central LoRA Library), `rolling_backup` OneTrainer (NON-ISSUE), et tout autre candidat de l'audit global post-Mission 150.
...
## v0.2-mission149 — 2026-09-24

*Note de régularisation* : cette entrée est rédigée pendant la régularisation documentaire post-publication de Mission 149 — commit fonctionnel, tag et Release sont déjà tous réels au moment de la rédaction.

### Résumé (Mission 149)

Issue du Candidate 1 de l'audit global READ-ONLY mené après clôture complète de Mission 148, validé par l'architecte, puis verrouillé par un mini-audit architectural READ-ONLY dédié comparant 4 options avant toute rédaction de mission (A : persister le chemin exact d'exposition sur le Domain `LoRA`, taille M ; B : registre d'exposition séparé, taille M, écarté comme sur-ingénierie — convention « pas de scaffolding avant un besoin réel », un seul root par provider étant le seul besoin réel actuel ; C : verrouiller les champs Settings d'exposition tant qu'une exposition Toolkit existe dans le root courant, taille S, retenue ; D : migration automatique au changement de Settings, taille L, écartée comme trop complexe/risquée pour un gain marginal). Root cause : la Central LoRA Library expose un LoRA par hardlink NTFS vers le root Forge ou ComfyUI courant (`LoRALibraryManager._expose()`/`_unexpose()`, Missions 095/108/135), mais aucune métadonnée d'exposition n'est jamais persistée nulle part (ni Domain `LoRA`, ni `LoRALibraryStorage`) — l'alias est localisé exclusivement via l'argument `expose_root` fourni en direct au moment de l'appel, lui-même lu en direct depuis `ApplicationSettings.forge_lora_expose_path`/`comfyui_lora_expose_path`. Modifier ensuite ce chemin en Settings faisait perdre à Toolkit toute connaissance pratique de l'ancien emplacement : une suppression ultérieure de cette même LoRA ne désexposait alors que depuis le nouveau root, supprimait le fichier canonique, et laissait l'ancien hardlink physiquement présent et utilisable par Forge/ComfyUI — partageant toujours le même inode que le fichier canonique désormais supprimé — sans aucune erreur affichée, reproductible en 3 actions UI ordinaires sans manipulation filesystem ni redémarrage.

Contrat retenu (Option C) : `ApplicationSettingsManager.update()` verrouille désormais indépendamment `forge_lora_expose_path` et `comfyui_lora_expose_path` — nouvelle exception partagée `LoRAExposureRootLockedError` (un seul type pour les deux providers, le provider concerné identifié uniquement par le texte du message, miroir du paramétrage `engine_label` déjà pratiqué par `_expose()`/`_unexpose()` plutôt que par multiplication de types d'exception) — dès qu'un changement de valeur réel est demandé alors qu'au moins une LoRA connue de la Central Library possède encore un alias identifiable dans le root actuellement configuré pour ce provider, détecté par une nouvelle primitive read-only `LoRALibraryManager.has_any_exposure(expose_root) -> bool` (glob `*__<lora_id>.*` réutilisant exactement `_find_existing_alias()`, aucune mutation, aucun `_save()`, aucune création de dossier). Forge et ComfyUI sont verrouillés indépendamment — verrouiller l'un n'affecte jamais l'autre. Resaisir exactement la valeur déjà configurée reste un no-op silencieux même en présence d'une exposition, mirroir exact du contrat déjà établi pour `lora_library_path` (`LoRALibraryPathLockedError`, Mission 087). Un fichier étranger au dossier d'exposition, ou un alias appartenant à un `lora_id` inconnu de la Central Library actuelle, ne verrouille jamais rien ; une ambiguïté multi-alias continue de se propager comme `LoRALibraryError`, jamais silencieusement résolue à `True` ni avalée. Les deux nouveaux gardes s'exécutent avant toute construction de `candidate`/tout `ApplicationSettingsStorage.save()` — un changement de root refusé dans un `update()` combiné ne persiste aucun autre champ soumis dans le même appel, prouvé par test dédié. `SettingsPage` réutilise exactement le pattern d'exception déjà existant pour `LoRALibraryPathLockedError` (message clair, resynchronisation du champ), sans nouveau widget. Aucun changement à `_expose()`/`_unexpose()`/`delete_from_library()`/Mission 135/Domain `LoRA`/`LoRALibraryStorage`/à la stratégie hardlink.

**Limite assumée** : Mission 149 est strictement prospective. Elle empêche la création de nouveaux hardlinks oubliés lors d'un futur changement de root, mais ne prétend pas retrouver ni supprimer les hardlinks historiques déjà présents dans des roots que les Settings ne connaissent plus — aucun scan filesystem global, aucune reconstruction par inode.

### Tests ajoutés (Mission 149)

**+20 tests nets** (2842 → 2862 tests collectés). 9 nouveaux dans `LoRALibraryManagerHasAnyExposureTest` (`tests/integration/test_lora_library_roundtrip.py`) : bibliothèque vide, root non configuré, root inexistant, dossier `AIStudioToolkit` absent, exposition connue détectée, no-op après désexposition, fichier étranger ignoré, alias appartenant à un `lora_id` inconnu ignoré, ambiguïté multi-alias toujours levée en `LoRALibraryError`. 11 nouveaux dans `ApplicationSettingsExposureRootLockTest` (même fichier) : Forge refusé/no-op/réouvert après désexposition (×3), ComfyUI en miroir (×3), verrou Forge sans effet sur ComfyUI et réciproquement (×2), atomicité d'un `update()` combiné refusé (aucun autre champ persisté), absence de verrou lorsqu'aucun `lora_library_manager` n'est injecté. Écart mineur documenté par rapport au draft de mission : les deux classes sont placées dans `test_lora_library_roundtrip.py` plutôt que `test_application_settings_roundtrip.py`, ce dernier fichier n'injectant jamais de `LoRALibraryManager` réel — exactement la même raison structurelle que le test du verrou `lora_library_path` préexistant, déjà logé au même endroit. Le commentaire obsolète de `test_application_settings_roundtrip.py` (lignes 685-689, affirmant que `comfyui_lora_expose_path` n'a « no lock of any kind ») a été corrigé pour refléter le nouveau contrat, sans aucun autre changement dans ce fichier.

### État du projet (Mission 149)

**2862 tests collectés.** Ciblés : `test_lora_library_roundtrip.py` **117/117**, `test_application_settings_roundtrip.py` **21/21**, `test_settings_page.py` **90/90**. Non-régression : `test_lora_roundtrip.py` (Mission 135) **275/275**, `test_training_roundtrip.py` **384/384**. **Suite complète : 2862 collectés/2862 passés/0 échoué.** Équation : 2842 (clôture Mission 148) + 20 nets ajoutés par Mission 149 = **2862**, cohérent. `git diff --check` clean. Exactement les 6 fichiers annoncés modifiés (3 production — `src/managers/lora_library_manager.py`, `src/managers/application_settings_manager.py`, `src/ui/pages/settings_page.py` — 2 test, 1 documentation) — aucun fichier étranger. Aucun smoke manuel requis, justifié : mécanisme entièrement démontré et corrigé au niveau du Manager/Settings, aucune UI nouvelle, le pattern UI réutilisé est déjà validé par Mission 087. Commit fonctionnel `98d9c3c5057edfd67757ac4d0011fb550d66ed72` (« Guard LoRA exposure root changes while exposed »), tag `v0.2-mission149`, GitHub Release publiée manuellement. Dette « LoRA exposure orphan/path change », identifiée CONFIRMED BUG user-reachable par l'audit global post-Mission 148, est désormais **résolue prospectivement par Mission 149** pour tout changement de root à venir — les hardlinks déjà orphelins dans un root oublié avant cette mission restent non récupérables automatiquement à partir de l'état actuel. Hors périmètre, confirmé et non traité : `ComfyUIEngine.wait_for_result()` ignorant le statut d'exécution terminal (Candidate 2, reste ouvert), Workspace create failure cleanup du premier `save()` de `create_without_publishing()` (Candidate 3, reste borné à ce premier `save()`, M137 ayant déjà corrigé le second), `TrainingManager._training_folder()`/`training_id`/`dataset_id` path validation (latente/défense en profondeur), caption sidecars silencieux, Training Resume/process identity, nettoyage asymétrique de `create_job()`, cascade filesystem Character (latente/future, contrôles Character New/Delete/List cachés depuis Mission 026, ne concerne pas la Central LoRA Library — indépendante d'une éventuelle cascade Character), flakiness Forge des tests réel-process (TEST DEBT), `rolling_backup` OneTrainer (NON-ISSUE/décision arrêtée), et tout autre candidat de l'audit global post-Mission 148.
...
## v0.2-mission145 — 2026-09-23

*Note de régularisation* : cette entrée est rédigée pendant la régularisation documentaire post-publication de Mission 145 — commit fonctionnel, tag et Release sont déjà tous réels au moment de la rédaction.

### Résumé (Mission 145)

Issue de la comparaison des candidats du nouvel audit global READ-ONLY mené après Mission 144, dont la recommandation générale (lifecycle de suppression Training) a été validée par l'architecte, suivie d'un audit de conception dédié avant toute implémentation. `TrainingManager.delete()` (`src/managers/training_manager.py`) était une mutation Domain pure — retrait de `character.trainings`, `active_training_id` remis à `None` si concerné, `WorkspaceManager.save()`, sans aucun appel filesystem ni vérification d'activité, confirmé par sa propre docstring d'avant-mission (« Domain-only mutation, no filesystem involved »). Deux défauts en résultaient : (A) rien n'empêchait de supprimer un Training dont un job était `STARTING`/`RUNNING` — le process OS réel continuait de tourner, et à sa terminaison `TrainingJobRunner` appelait `update_job_state()`, qui résolvait le job via `_find_job()` (itérant `self.trainings`, désormais privé du Training supprimé) et retournait `False` silencieusement, avant toute mutation d'état, tout `save()`, toute publication d'événement — perte silencieuse du résultat final du job ; (B) `training/<training_id>/` (configs, sorties, checkpoints, backups OneTrainer) n'était jamais supprimé du disque, dette déjà documentée deux fois sans être traitée (`MISSION_138.md` §20/§26, `MISSION_139.md` §15).

Un audit pré-implémentation obligatoire a établi que `has_active_job()` (Mission 100, garde de fermeture d'application) scanne tous les Trainings du Character principal — le réutiliser tel quel pour `delete()` aurait bloqué à tort la suppression d'un Training inactif simplement parce qu'un autre Training sans rapport possède un job actif. Un nouveau prédicat local, scopé exclusivement au Training ciblé (`any(job.state in TRAINING_JOB_ACTIVE_STATES for job in training.jobs)`), a donc été retenu, `has_active_job()` restant strictement inchangée. Nouvelle exception `TrainingActiveError`, levée par `delete()` avant toute mutation Domain ou filesystem si le Training ciblé a un job `STARTING`/`RUNNING` — refus net, aucun `QMessageBox`/annulation automatique/attente `QProcess`/orchestration asynchrone dans le Manager, l'utilisateur devant utiliser le mécanisme Cancel existant puis réessayer.

`training/<training_id>/` (un seul sous-arbre couvrant Training-level et tous ses `jobs/<job_id>/`) est désormais supprimé transactionnellement en mirroring exact de `LoRAManager.delete()` (Character-scoped, pattern le plus proche structurellement) : move atomique vers `.trash/training_<id>_<uuid>/` avant toute mutation Domain (échec ici : rien touché) ; mutation Domain puis `WorkspaceManager.save()` (échec : restauration Domain toujours exécutée en premier, puis reverse-rename du dossier, avec `WorkspaceManagerError` enrichie chaînée en cas de double échec) ; sur succès, purge best-effort du dossier `.trash/` sans jamais rollback le Domain déjà persisté en cas d'échec de purge — rapporté uniquement via le nouveau `TrainingDeletionResult(deleted, cleanup_failed, residual_path)`, même convention que `LoRADeletionResult`/`DatasetDeletionResult` (Mission 075). Tracé dans le code et non supposé : `import_lora()` copie toujours physiquement le fichier de sortie dans la Central LoRA Library (arbre totalement distinct du Workspace), et l'exposition Forge/ComfyUI hardlink toujours depuis cette copie — supprimer `training/<training_id>/` est donc sûr pour tout LoRA déjà importé et son exposition.

Un audit exhaustif des call sites de `delete()` (exigé avant clôture, le contrat public changeant de `bool` à `TrainingDeletionResult` — un `NamedTuple` toujours vrai en Python quel que soit son contenu) a trouvé et corrigé une occurrence vacuité exacte (`TrainingManagerDeleteRollbackTest.test_delete_succeeds_normally_when_save_works`, `assertTrue(result)` → `assertTrue(result.deleted)`). Une comparaison exhaustive avec les trois callers UI existants d'un `*DeletionResult` (`DatasetsPage.delete_dataset()`, `LoRAPage.delete_lora()`/`delete_from_library()`) a confirmé une convention produit déjà établie — un `QMessageBox.warning(self, "Suppression partielle", ...)` non bloquant sur `cleanup_failed` — que `TrainingPage.delete_training()` suit désormais à l'identique, absente de la première implémentation et ajoutée après cette vérification. La confirmation de suppression avertit désormais explicitement de la suppression des fichiers/résultats locaux tout en précisant qu'un LoRA déjà importé dans la Bibliothèque LoRA n'est pas supprimé. Aucun tracking "output importé" introduit (aucun lien Domain fiable pour le déterminer, explicitement rejeté en conception).

### Tests ajoutés (Mission 145)

**+16 tests nets** (2808 → 2824 tests collectés). Nouvelle classe `TrainingManagerDeleteFilesystemAndActiveJobTest` (`tests/integration/test_training_roundtrip.py`, 11 tests) : delete normal avec filesystem réel supprimé, dossier absent toléré, refus sur job `STARTING`/`RUNNING`, Training B supprimable pendant que Training A a un job actif (verrouille la correction de granularité de la garde), échec du move-to-trash (aucune mutation Domain/filesystem), échec de `save()` avec restauration Domain **et** filesystem prouvée, échec de `save()` **et** du reverse-rename (message chaîné avec récupération manuelle), échec de purge après `save()` réussi (`deleted=True`, `cleanup_failed=True`), Training multi-job (un seul move capture tous les jobs), `TRAINING_DELETED` publié uniquement sur succès complet. `TrainingPageDeleteConfirmationTest` (4 tests nets) : texte de confirmation mentionnant fichiers/résultats locaux et survie de la Bibliothèque LoRA, `TrainingActiveError` intercepté par un `QMessageBox.warning` dédié (Domain/filesystem intacts), warning « Suppression partielle » sur `cleanup_failed=True` avec `residual_path` exact, absence de warning sur succès complet. `TrainingPageJobImportTest` (1 test net) : Central LoRA Library démontrée indépendante après suppression du Training source. 4 tests déjà existants transformés (`.deleted` au lieu de la vérité simple de l'ancien booléen), aucune assertion fonctionnelle nouvelle — dont un piège exact de vacuité trouvé et corrigé pendant l'audit call-site (voir Résumé).

### État du projet (Mission 145)

**2824 tests collectés.** `tests/integration/test_training_roundtrip.py` complet (fichier ciblé, contient tous les tests M145) : **384/384**. Suites voisines exécutées isolément : `test_training_job_runner.py` **12/12**, `test_workspace_roundtrip.py` **107/107**, `test_lora_library_roundtrip.py` **97/97**, `test_lora_roundtrip.py` **275/275** (référence de la convention `cleanup_failed`, non modifiés — non-régression confirmée). **Suite complète : 2824 collectés/2824 passés/0 échoué, exit 0 (336.977s)** — un seul traceback visible dans le log, bénin et déjà pré-existant (`inference_page.py:789/865`, `QLabel.setText(MagicMock)`, sans rapport avec ce diff, non-régression déjà documentée par Missions 142/143). Équation : 2808 (clôture Mission 144) + 16 nets ajoutés par Mission 145 = **2824**, cohérent. `git diff --check` clean. Exactement les 4 fichiers annoncés modifiés (2 production, 1 test, 1 documentation) — aucun fichier étranger. Aucun smoke manuel requis, justifié : mécanisme entièrement synchrone et déterministe côté Domain/filesystem (mêmes primitives `WorkspaceStorage.rename_folder()`/`delete_folder()` déjà exercées par Dataset/LoRA), aucune UI Qt nouvelle au-delà d'un texte et d'un `QMessageBox.warning` supplémentaire déjà couverts par test automatisé. Commit fonctionnel `3dcc625f02bca4e9398c1db8c933550058513582` (« Guard Training deletion lifecycle »), tag `v0.2-mission145`, GitHub Release publiée manuellement. Hors périmètre, confirmé et non traité : Training Resume, PID/process identity, `rolling_backup` OneTrainer, nettoyage des 4 dossiers historiques `create_job()`, cascade filesystem Character, nettoyage `WorkspaceManager.create_without_publishing()`, sidecar caption, crash `RUNNING_OWNED` Forge/ComfyUI, redesign Central LoRA Library, tracking "output importé", framework transactionnel générique. Nouvelle dette découverte et documentée, non corrigée : `TrainingManager._training_folder()` ne valide pas `training_id` contre le path traversal et suppose un Workspace ouvert.

---

## v0.2-mission144 — 2026-09-22

*Note de régularisation* : cette entrée est rédigée pendant la régularisation documentaire post-publication de Mission 144 — commit fonctionnel, tag et Release sont déjà tous réels au moment de la rédaction.

### Résumé (Mission 144)

Issue de la comparaison des candidats du nouvel audit global READ-ONLY mené après Mission 143, retenue pour sa priorité (1) perte/corruption de données dans le référentiel de priorités explicitement retenu pour cet audit — seule candidate de cet audit relevant de cette catégorie —, sa certitude maximale (contraste direct contre `WorkspaceStorage.load()`, déjà correct dans le même dépôt pour les mêmes exceptions, non par inférence) et son risque le plus faible (purement défensif, aucun changement du chemin de succès). `ApplicationSettingsStorage.load()` (`src/infrastructure/storage/application_settings_storage.py`) et `LoRALibraryStorage.load()` (`src/infrastructure/storage/lora_library_storage.py`) capturaient `json.JSONDecodeError`/`OSError`, journalisaient un simple `logger.warning`, puis retournaient `None` — exactement la même valeur que pour un fichier absent (premier lancement légitime), alors que `WorkspaceStorage.load()` gère déjà les mêmes exceptions en levant `WorkspaceStorageError`. `ApplicationSettingsManager.__init__()`/`LoRALibraryManager.__init__()` interprétaient ce `None` comme « premier lancement » et repartaient silencieusement sur des valeurs par défaut (`ApplicationSettings()` vide, `self._loras = []`).

Un audit de conception pré-implémentation obligatoire, validé par l'architecte en plusieurs étapes séparées avant tout code, a démontré bout en bout le chemin d'écrasement différé pour les deux Storages plutôt que de le supposer : un seul `ApplicationSettingsManager.update(champ_quelconque=...)` ou un seul `LoRALibraryManager.import_lora()`/`update()`/`delete()` réussi après un chargement corrompu réécrit l'intégralité du fichier avec les valeurs post-corruption, écrasant définitivement la configuration ou le catalogue réels précédents à la prochaine sauvegarde routinière. Les deux `load()` lèvent désormais une exception Storage dédiée (déjà existante, docstring élargie pour couvrir la lecture) sur JSON invalide, `OSError` de lecture, et racine JSON syntaxiquement valide mais non-dict ; `LoRALibraryStorage.load()` lève en plus la même exception quand la clé `loras` est présente mais n'est pas une liste — une clé `loras` absente (`{}`) reste tolérée comme catalogue vide, et les entrées individuellement malformées à l'intérieur d'une liste par ailleurs valide restent tolérées exactement comme avant, comportement déjà existant et déjà testé, strictement inchangé par cette mission. Aucun `save()` n'a été modifié : les trois Storages du dépôt suivaient déjà le schéma `tempfile → flush → fsync → os.replace()`, `ApplicationSettingsStorage`/`LoRALibraryStorage` l'ayant même avant Mission 140 (qui s'en est inspirée pour durcir `WorkspaceStorage.save()`).

Une première proposition d'architecture de démarrage, faisant porter la gestion d'erreur à `MainWindow` elle-même, a été explicitement rejetée par l'architecte pour avoir réparti la responsabilité d'un même incident entre deux composants. L'architecture retenue exploite le fait que `ApplicationSettingsManager.__init__()`/`LoRALibraryManager.__init__()` n'avaient déjà aucun `try/except` autour de leur appel `Storage.load()` : l'exception remonte donc, sans aucune modification de `src/ui/main_window.py` (confirmé inchangé), jusqu'à `core/main.py::main()`, seul point d'interception — un `QMessageBox.critical(None, ...)` fatal distinct par cause (Application Settings vs LoRA Library), suivi d'un `return` (pas de `SystemExit`, choix délibéré pour ne pas changer le contrat public de `main()` en l'absence de tout consommateur du code de sortie), avant tout `window.show()`/`app.exec()`. Le comportement réel d'un `QMessageBox.critical(None, ...)` avant `app.exec()` a été vérifié empiriquement avec un vrai PySide6 pendant la conception (script jetable, hors dépôt), conformément à l'exigence explicite de ne pas se contenter d'une hypothèse théorique. Ni l'exception, ni le message, ne suppriment, ne renomment, ni ne recréent de fichier — le fichier corrompu reste octet pour octet intact, prouvé par test dédié.

### Tests ajoutés (Mission 144)

**+10 tests nets** (2798 → 2808 tests collectés). `tests/integration/test_application_settings_roundtrip.py` : `test_storage_load_compatibility_matrix` transformé (`assertRaises` au lieu de `assertIsNone` pour tous les cas malformés, ajout du cas racine `null`, vérification de `__cause__` pour l'`OSError`) plus 2 tests nets nouveaux (`test_storage_load_malformed_json_preserves_original_cause`, `test_storage_load_failure_never_modifies_the_corrupt_file` — preuve bytes avant/après). `tests/integration/test_lora_library_roundtrip.py` : deux tests existants renommés et transformés (`test_invalid_json_raises_storage_error_with_error_log`, `test_non_dict_root_raises_storage_error`) plus 4 tests nets nouveaux (`test_non_list_loras_value_raises_storage_error`, `test_missing_loras_key_still_tolerated_as_empty_catalog`, `test_read_oserror_raises_storage_error_with_cause_preserved`, `test_load_failure_never_modifies_the_corrupt_file`) — les deux tests déjà existants sur la tolérance des entrées individuelles malformées restent inchangés et toujours verts. `tests/integration/test_main_startup.py` (nouveau fichier) : 4 tests nets nouveaux couvrant pour la première fois `core/main.py` (`QApplication`/`MainWindow`/`QMessageBox` patchés pour éviter un second `QApplication([])` réel dans un process de test qui en partage déjà une instance) — un test par cause d'exception, un test confirmant que les deux messages sont distinguables, un test de non-régression du chemin de succès.

### État du projet (Mission 144)

**2808 tests collectés.** Les 3 fichiers de test ciblés Mission 144 ensemble : **122/122**. Suite `test_main_window_*.py` (10 fichiers, non-régression, fichier non modifié) : **136/136**. Suite complète : **2808 collectés/2808 passés/0 échoué, exit 0 (340.594s)** — aucun flake sur ce run. Équation : 2798 (clôture Mission 143) + 10 nets ajoutés par Mission 144 = **2808**, cohérent. `git diff --check` clean. Exactement les 7 fichiers annoncés modifiés (3 production, 2 tests modifiés, 1 nouveau test, 1 nouvelle documentation) — aucun fichier étranger. Aucun smoke manuel requis, justifié : le comportement `QMessageBox.critical(None, ...)` avant `app.exec()` a été observé réellement avec PySide6 pendant la conception, et le comportement du composition root est couvert par test automatisé. Commit fonctionnel `f73797c200fd85b25991c6e32cbba38465052d85` (« Prevent silent fallback on corrupt storage »), tag `v0.2-mission144`, GitHub Release publiée manuellement. Hors périmètre, confirmé et non traité : `from_dict()` permissif (Settings et LoRA), validation complète de schéma, récupération automatique, `.bak`/rename automatique, degraded mode, `src/ui/main_window.py`, `TrainingManager.delete()` filesystem, cascade filesystem Character, nettoyage `WorkspaceManager.create_without_publishing()`, sidecar caption, rolling backup OneTrainer, Training Resume, Forge, ComfyUI, EventBus, Settings réservés (`python_path`/`ollama_path`).
...
## v0.2-mission142 — 2026-09-21

*Note de régularisation* : cette entrée est rédigée pendant la régularisation documentaire post-publication de Mission 142 — commit fonctionnel, tag et Release sont déjà tous réels au moment de la rédaction.

### Résumé (Mission 142)

Issue du nouvel audit global READ-ONLY mené après Mission 141, classé priorité (2) — faux succès/faux échec — combiné à un lifecycle process incomplet, dans le référentiel de priorités explicitement retenu pour cet audit : la même classe de défaut que celle corrigée par Mission 141 dans `ForgeLifecycleManager`, mais reconstruite indépendamment sur le code `ComfyUILifecycleManager` réel, sans copier-coller aveugle du raisonnement Forge. `ComfyUILifecycleManager` (`src/ui/comfyui_lifecycle_manager.py`) est instancié une seule fois pour toute la session applicative (`src/ui/main_window.py:195`), partagée avec `SettingsPage`/`InferencePage` — même architecture single-instance-per-session que Forge. `_terminate_owned_process()` crée, à chaque tentative d'arrêt réelle, un `QTimer` borné à `TERMINATE_TIMEOUT_SECONDS` (10.0s), jamais explicitement arrêté lorsque le process répond de lui-même à `terminate()` avant l'échéance (cas normal) — ni dans `_on_process_finished()`, ni dans `_finish_process_teardown()`, les deux seuls points de résolution réels du teardown ComfyUI. Contrairement à Forge, ComfyUI ne possède ni process `taskkill` séparé, ni rendez-vous à deux flags, ni méthode unique `_maybe_finish_teardown()` — la correction a donc nécessité deux points d'insertion distincts plutôt qu'un seul choke-point. `_on_terminate_timeout()` ne possédait aucune protection contre un signal tardif d'un cycle antérieur, contrairement au garde d'identité déjà établi dans ce même fichier pour le worker de readiness (`_on_readiness_ready()`/`_on_readiness_timed_out()`).

Un audit pré-implémentation obligatoire a reconstruit le scénario cross-cycle exact sur ComfyUI : Cycle A se termine normalement pendant `RUNNING_OWNED` (`terminate()` réussit avant l'échéance de `timer_A`, jamais arrêté) ; peu après, Cycle B démarre un process légitime sur la même instance sans jamais rappeler `_terminate_owned_process()` (donc `self._terminate_timer` n'est jamais réassigné, il pointe toujours sur `timer_A`) ; à échéance, `timer_A.timeout` s'émet tardivement et invoque `_on_terminate_timeout()`, qui tue le process légitime de Cycle B (`process_B.kill()`). Deux variantes selon l'état exact de B : si B est encore `STARTING`, `_on_process_finished()` produit un faux `START_FAILED` avec un message trompeur (« process exited before becoming available ») ; si B a déjà atteint `RUNNING_OWNED`, `_on_process_finished()` ne correspond à aucune de ses deux branches (`STARTING`/`STOPPING`) et ne fait strictement rien — l'état reste `RUNNING_OWNED` indéfiniment alors que le process réel est mort, sans aucun message d'erreur, une variante silencieuse documentée comme sous-cas du même bug. L'audit a également confirmé qu'un garde d'identité seul, sans arrêt du timer, aurait été insuffisant pour la même raison que sur Forge : `_terminate_timer` n'est réassigné que lorsque le cycle courant lui-même appelle `_terminate_owned_process()`, ce que Cycle B, un Start légitime, ne fait jamais.

Mission 142 corrige les deux causes de façon strictement additive, avec deux points d'insertion adaptés à l'architecture ComfyUI : (1) `_on_process_finished()` et `_finish_process_teardown()` arrêtent et dé-référencent désormais explicitement `_terminate_timer` dès que le teardown se résout par l'une ou l'autre voie ; (2) `_on_terminate_timeout()` reçoit un garde d'identité en miroir exact du pattern déjà établi pour le worker de readiness. Aucun changement de constante de timeout, d'API publique, de sémantique d'échec ou de structure de fichier. Un risque intra-cycle dérivé (double appel à `_terminate_owned_process()` pendant la fenêtre `STARTING`) est documenté comme observation hors périmètre, neutralisée en bonus par le garde d'identité mais non corrigée à sa source.

### Tests ajoutés (Mission 142)

**+6 tests nets** (2786 → 2792 tests collectés) dans la classe existante `ComfyUILifecycleManagerGuardTest` (`tests/integration/test_comfyui_lifecycle_manager.py`) : `test_on_process_finished_stops_and_clears_the_terminate_timer_once_resolved` et `test_finish_process_teardown_stops_and_clears_a_residual_terminate_timer` — preuve, avec de vrais `QTimer`, que chacun des deux chemins de résolution arrête et dé-référence bien le timer ; `test_stale_terminate_timeout_signal_is_ignored` — un signal dont le sender n'est pas le timer courant n'a aucun effet ; `test_terminate_timeout_from_the_current_timer_is_not_treated_as_stale` — non-régression, le timer courant continue de déclencher normalement le fallback `kill()` ; `test_stale_terminate_timeout_from_a_resolved_stop_never_affects_a_later_start` — test de régression principal reproduisant fidèlement le scénario cross-cycle complet (variante `STARTING`), prouvant un effet métier réel : le process de Cycle B n'est jamais tué, son état reste inchangé ; `test_stale_terminate_timeout_from_a_resolved_stop_never_kills_a_later_running_owned_process` — même preuve pour la variante silencieuse `RUNNING_OWNED`.

### État du projet (Mission 142)

**2792 tests collectés.** `ComfyUILifecycleManagerGuardTest` seule : **23/23** (17 existants inchangés + 6 nouveaux). `test_comfyui_lifecycle_manager.py` complet (2 classes, y compris les tests process-réel) : **31/31**, aucun flake sur ce run. `test_main_window_close_event.py` : **46/46**, non-régression confirmée. `test_inference_page.py` + `test_settings_page.py` (non-régression) : **320/320** (un traceback bénin pré-existant et sans rapport avec ce diff, `inference_page.py:789`/`:865`, `QLabel.setText(MagicMock)`, imprimé pendant le run sans provoquer d'échec). Suite complète : **2792 collectés/2792 passés/0 échoué, exit 0 (334.878s)** — aucun flake sur ce run. Équation : 2786 (clôture Mission 141) + 6 nets ajoutés par Mission 142 = **2792**, cohérent. `git diff --check` clean. Exactement les 3 fichiers annoncés modifiés (1 production, 1 test, 1 documentation) — aucun fichier étranger. Smoke réel ComfyUI non effectué, justifié : le bug est intégralement démontré et corrigé au niveau du lifecycle Qt automatisé avec de vrais objets `QTimer`, et le comportement du process réel reste couvert, inchangé et vert, par `ComfyUILifecycleManagerRealProcessTest` déjà existante. Commit fonctionnel `5252066083d66ef51f6816df46ee469b26361fff` (« Guard ComfyUI teardown against stale terminate timers »), tag `v0.2-mission142`, GitHub Release publiée manuellement. Hors périmètre, confirmé et non traité : Forge (déjà clos par Mission 141, aucune modification), rolling backup OneTrainer, Resume Training, nettoyage filesystem de `TrainingManager.delete()`, dette historique des 4 dossiers `create_job()`, Settings morts, backup/versioning de `project.json`, gestion d'erreur des sidecars caption, événements Training sans subscriber, toute refonte générale du lifecycle ComfyUI au-delà du correctif ciblé, la variante intra-cycle dérivée documentée en §2 de `MISSION_142.md`.
...
## v0.2-mission139 — 2026-09-21

*Note de régularisation* : cette entrée est rédigée pendant la régularisation documentaire post-publication de Mission 139 — commit fonctionnel, tag et Release sont déjà tous réels au moment de la rédaction.

### Résumé (Mission 139)

Première mission d'implémentation issue de la conception livrée par **Mission 138 — Investigate Training Crash/Interruption Recovery** (mission d'audit/conception pure, sans code produit ni test, commit documentaire séparé `7d6f5752668fa5fb5c875ff112783a95e267f432` — « Document Mission 138 training recovery design » —, sans tag ni Release propres, voir `docs/missions/MISSION_138.md`). Cet audit avait établi l'orientation générale d'une future reprise (Resume) d'entraînement OneTrainer interrompu (Architecture B — un nouveau `TrainingJob` isolé par tentative, lié au précédent via `resumed_from_job_id`) et identifié, dans le même travail, un défaut d'isolation filesystem concret et **indépendant** du sujet Resume : le dossier concept matérialisé qu'OneTrainer lit réellement (images + légendes `.txt`) était partagé au niveau `Training`, jamais isolé par `TrainingJob` — `TrainingManager._materialize_concept()` réécrit inconditionnellement ce dossier à chaque clic sur Démarrer, pour n'importe quel Job du Training, et `create_job()` ne faisait jusqu'ici que reporter tel quel le chemin de ce dossier partagé dans la configuration OneTrainer figée, sans jamais le copier — un second Job du même Training pouvait donc silencieusement invalider les données qu'un premier Job, déjà créé, croyait figées.

Mission 139 corrige exactement ce défaut, sans implémenter aucune fonctionnalité de Resume. Chaque `TrainingJob` possède désormais son propre snapshot concept, isolé exactement comme `output`/`workspace`/`cache`/`debug` depuis Mission 100 : nouveau champ `TrainingJobPaths.concept_dir` (`training/<training_id>/jobs/<job_id>/concept/`), `create_job()` copie fichier par fichier (`shutil.copy2`, même primitive que `_materialize_concept()`) le contenu du dossier concept Training-scope déjà matérialisé par le Prepare requis en amont — uniquement les images et les sidecars `.txt` déjà présents, rien d'autre — puis réécrit `config["concepts"][0]["path"]` dans la copie figée avant de l'écrire sur disque. Le point critique a été vérifié par test dédié relisant directement le JSON réellement sérialisé sur disque pour chaque Job, pas seulement l'existence de deux dossiers distincts. Preuve principale de la mission : Job A créé et son snapshot capturé, Dataset modifié et `trigger_word` changé, Job B créé après un nouveau Prepare — le snapshot de A reste strictement identique (mêmes fichiers, même contenu), B reflète le nouvel état, et la configuration figée de chacun pointe toujours vers son propre `concept_dir`.

Une correction a été demandée en revue ChatGPT avant clôture Git : à la différence des 4 dossiers historiques de `create_job()` (`output`/`workspace`/`cache`/`debug`, convention déjà en vigueur et inchangée — « additive, per-job-id folder, harmless to leave orphaned if save() subsequently fails »), un échec de copie du concept snapshot — artefact **nouvellement introduit** par cette mission — n'est plus toléré de la même façon. La création/copie du `concept_dir` est désormais isolée dans son propre bloc `try/except OSError`, exécuté avant toute création des 4 autres dossiers et avant toute construction/persistance du `TrainingJob` : sur échec, nettoyage best-effort du seul `concept_dir` partiel via `WorkspaceStorage.delete_folder()` (jamais `shutil.rmtree(ignore_errors=True)`), aucun autre dossier — ni un autre Job, ni le dossier source partagé — jamais touché, aucun `TrainingJob` jamais persisté, `TrainingJobError` levée avec la cause primaire préservée (`raise ... from exc`), et si le nettoyage lui-même échoue, le message diagnostique les deux échecs sans jamais remplacer la cause primaire (même idiome que `_materialize_concept()`/`LoRALibraryManager.import_lora()`). Aucun changement `Training`/`TrainingJob` Domain, machine à états, EventBus, UI, Central LoRA Library, Inference, backup OneTrainer, Resume ou PID/process management — `src/engines/onetrainer_config.py` non modifié. Une dette a été découverte et documentée mais délibérément non traitée par cette mission, conformément au périmètre strict demandé : les 4 dossiers historiques de `create_job()` partagent la même absence de nettoyage sur échec que le `concept_dir` avait avant correction.

### Tests ajoutés (Mission 139)

**+7 tests nets** (2772 → 2779 tests collectés) dans `TrainingManagerCreateJobTest` (`tests/integration/test_training_roundtrip.py`, 19/19) : isolation du `concept_dir` par Job (distinct du dossier partagé) ; deux Jobs du même Training n'obtiennent jamais le même `concept_dir` ; le snapshot contient bien l'image et le sidecar `.txt` (contenu vérifié) ; la configuration OneTrainer réellement sérialisée référence le propre chemin du Job, jamais le partagé ; la création/préparation d'un second Job ne mute jamais le snapshot du premier même après changement du Dataset et du `trigger_word` (preuve principale) ; un échec de copie (`shutil.copy2` patché) lève `TrainingJobError`, ne persiste aucun Job et ne laisse aucun `concept_dir` résiduel ; un double échec simulé (copie **et** nettoyage patchés pour échouer) diagnostique les deux causes sans jamais remplacer la cause primaire dans `__cause__`. `tests/integration/test_training_job_runner.py` : 2 constructions manuelles existantes de `TrainingJobPaths(...)` complétées avec le nouveau champ obligatoire `concept_dir`, aucune assertion modifiée (12/12 inchangé). `test_training_roundtrip.py` complet **368/368**, `test_onetrainer_config.py` complet **169/169** (fichier non modifié, non-régression confirmée).

### État du projet (Mission 139)

**2779 tests collectés.** Deux exécutions complètes indépendantes ont été effectuées avant clôture. La première a obtenu 2779 collectés/2778 passés/1 échoué, isolé à `test_forge_lifecycle_manager.py::ForgeLifecycleManagerReadinessTimeoutCleanupConfirmationTest.test_readiness_timeout_with_taskkill_success_confirms_cleanup` — reproduit vert en isolation (fichier complet 48/48, test isolé 3/3). Sur demande explicite avant commit, une seconde exécution complète a été relancée et a obtenu le même résultat global, 2779 collectés/2778 passés/1 échoué, mais isolé cette fois à un test **différent** du même fichier/de la même famille de tests processus-réel/timing, `ForgeLifecycleManagerRealProcessTest.test_stop_on_running_owned_kills_the_real_process_tree` — également reproduit vert en isolation (fichier complet 48/48, test isolé 3/3). Les deux incidents sont documentés honnêtement ici plutôt que masqués : ils sont cohérents avec la même famille de flake historique déjà connue pour ce fichier (tests dépendant d'un processus OS réel et de délais), sans aucun rapport avec le diff de cette mission (le fichier n'a jamais été touché) — la clôture a été explicitement acceptée sur cette base, sans troisième exécution complète, conformément à la consigne de ne pas boucler les full runs indéfiniment une fois le même flake historique reproduit une seconde fois. `git diff --check` clean. Exactement les 4 fichiers annoncés modifiés (1 production, 2 tests, 1 documentation) — aucun fichier étranger. Aucun smoke manuel requis : le point critique (configuration OneTrainer réellement sérialisée référençant le bon snapshot concept) est vérifié directement par test automatisé. Commit fonctionnel `d78b32ceb05b2e4a49b74850fba4eba59705056d` (« Isolate materialized training concepts per job »), tag `v0.2-mission139`, GitHub Release publiée manuellement. Le commit documentaire séparé de Mission 138 (`7d6f5752668fa5fb5c875ff112783a95e267f432`) précède ce commit fonctionnel sur `main`, sans tag ni Release propres.
...
## v0.2-mission087 — 2026-08-29

*Note de régularisation* : cette entrée est rédigée pendant la régularisation documentaire post-publication de Mission 087 — commit, tag et Release sont déjà tous réels au moment de la rédaction.

### Résumé (Mission 087)

L'audit mené après la clôture de Mission 086 a réévalué le besoin « bibliothèque LoRA centralisée multi-engine », déjà enregistré et partiellement décidé lors de la régularisation de Mission 060 (stockage physique centralisé et unique, appartenance logique séparée du stockage physique, trois scopes `Character`/`Workspace`/`Global-Shared`, exposition moteur différée à un futur audit dédié). Un premier mini-audit a confirmé, à partir du code réel, que `LoRA` est aujourd'hui nichée dans `Character.loras` sans collection top-level ni Manager Workspace-owned ; que `LoRA.files` ne sont jamais copiés physiquement (références externes brutes), seul `LoRA.thumbnail` étant physiquement possédé sous `workspace_root/models/loras/<lora_id>/` ; qu'aucune persistance hors Workspace n'existe dans ce dépôt à une seule exception, `ApplicationSettingsStorage` ; et que deux systèmes « LoRA » déjà déconnectés coexistent (`Character.loras`, jamais consommé par une génération, et `ApplicationSettings.comfyui_lora_name`/`comfyui_lora_strength`, seul réellement utilisé par `GenerationManager`). L'architecte a ensuite tranché quatre décisions structurantes (modèle Domain unifié sans type `SharedLoRA` parallèle ; suppression défensive sans index global cross-Workspace ; grandfathering, aucune migration physique forcée des LoRA existantes ; stockage binaire configurable, jamais `%LOCALAPPDATA%` imposé), puis validé qu'une fondation minimale (registre + configuration du chemin), sans toucher à `Character.loras`/`LoRAManager`/`project.json`/l'UI existante/l'exposition moteur, est le périmètre correct pour Mission 087. Un dernier mini-audit contractuel a fermé les questions de stockage restantes (emplacement exact du registre, réutilisation prudente de `WorkspaceStorage`, contrat précis du Manager, valeur par défaut du chemin binaire, politique de changement de chemin).

**Implémentation** : `LoRALibraryStorage` (nouveau) persiste un registre séparé `%LOCALAPPDATA%\AIStudioToolkit\lora_library.json` (même répertoire qu'`ApplicationSettingsStorage`, délégation via `default_directory()`, écriture atomique, tolérance fichier absent/JSON invalide). `LoRALibraryManager` (nouveau) réutilise intégralement le dataclass `LoRA` existant — `list_loras() -> list[LoRA]`, `get(lora_id) -> Optional[LoRA]`, `import_lora(name, file_paths, library_root, thumbnail_path=None) -> LoRA` (jamais `Optional`, vérifié contre le précédent réel `ModelManager.create()`), `delete(lora_id, library_root) -> LoRALibraryDeletionResult` — et copie réellement chaque fichier importé dans `<library_root>/<lora_id>/`, contrairement au contrat actuel de `Character.loras`. Contrat transactionnel dès la création : copie avant toute mutation Domain (abandon + nettoyage best-effort si une copie échoue), suppression via `.trash/` (mirroir exact des Missions 066/067/075), aucun état de chemin conservé par le Manager (`library_root` passé explicitement à chaque appel mutant, évitant toute dépendance circulaire avec `ApplicationSettingsManager`). Nouveau champ `ApplicationSettings.lora_library_path` (défaut `%USERPROFILE%\AI Studio Toolkit\LoRA Library`, écarté `%LOCALAPPDATA%` et `Documents` — risque de synchronisation OneDrive automatique) avec un champ Settings réel (saisie libre + bouton « Parcourir »). Politique de changement de chemin définitive : modifiable librement tant que le registre central est vide, refusée avec un message explicite dès qu'il contient au moins une entrée, aucune migration/copie/déplacement automatique jamais tentée, resaisir la valeur déjà configurée restant un no-op autorisé même registre non vide. Le verrou (nouvelle exception `LoRALibraryPathLockedError`) vit dans `ApplicationSettingsManager.update()` — seul point de passage réel de la persistence `ApplicationSettings`, jamais contournable depuis `SettingsPage`. Réutilisation précise de `WorkspaceStorage.copy_into_workspace(source, destination_folder, workspace_root=destination_folder)` (jamais la racine entière de la bibliothèque) pour préserver l'ownership strict par dossier d'entrée. Mission strictement fondationnelle et déconnectée : `Character.loras`, `LoRAManager`, `project.json`, `LoRAPage`, Inference et tout moteur (ComfyUI inclus) restent entièrement inchangés.

### Tests ajoutés (Mission 087)

- **40 tests ciblés nets nouveaux** : `test_lora_library_roundtrip.py` (39, nouveau fichier) — `LoRALibraryStorageTest` (registre absent/JSON invalide/non-dict, délégation de `default_directory()`, écriture atomique), `LoRALibraryManagerImportTest` (import mono/multi-fichiers, thumbnail optionnelle, collision de noms, deux imports de la même source → deux entrées et deux copies physiques distinctes sans déduplication, sources jamais modifiées/supprimées, échec de copie partielle et de persistence avec et sans échec du nettoyage de secours), `LoRALibraryManagerDeleteTest` (suppression réussie, ID inconnu, échec de déplacement/persistence avec restauration, double échec, cleanup final en échec sans jamais annuler une suppression déjà persistée, confinement strict au dossier propre à l'entrée), `LoRALibraryManagerListGetTest` (`list_loras()` retourne des `LoRA`, jamais des `dict`), `LoRALibraryPersistenceRoundTripTest` (persistence entre instances de Manager), `ApplicationSettingsLoraLibraryLockTest` (les 6 points du contrat de verrouillage). `test_application_settings_roundtrip.py` (1 net nouveau parmi 17 au total) — champ `lora_library_path` intégré à chaque test existant exhaustif.
- Non-régression complète : `test_application_settings_roundtrip.py` (17/17), `test_settings_page.py`/`test_settings_roundtrip.py` (90/90), `test_lora_roundtrip.py` (173/173, `LoRAManager` Character-scoped entièrement inchangé), suite `MainWindow` ciblée (122/122).
- **1630/1630 tests verts au total** (1590 précédents + 40 nets nouveaux), une exécution complète `unittest discover` isolée sans charge concurrente, 191,7 s, aucun crash. Deux défaillances isolées observées lors d'exécutions combinées à forte charge système auto-infligée (`InferencePageGenerationActiveGuardTest.test_confirm_does_not_touch_thread_or_pending_when_refusing`, hérité de Mission 085, et `test_close_refused_immediately_while_generation_genuinely_active`, hérité de Mission 085) ont été reproduites en isolation et confirmées vertes (3,2 s et 12,4 s respectivement) — anomalie de contention ponctuelle déjà documentée depuis Mission 085/086, non un défaut introduit par cette mission.
- Smoke test Qt réel, exécuté par Claude, `SettingsPage`/`ApplicationSettingsManager`/`LoRALibraryManager` réels, dossiers temporaires réels — **PASS, 12/12 assertions** (stable sur 3 exécutions consécutives) : valeur par défaut réelle affichée, clic réel sur Parcourir mettant à jour le champ, sauvegarde réelle persistée sur disque, rechargement depuis une nouvelle instance de Manager restaurant la valeur, refus réel une fois qu'une vraie entrée existe dans la bibliothèque.

### État du projet (Mission 087)

1630/1630 tests automatisés verts. Commit fonctionnel `dcc27ca` (`feat: add central LoRA library foundation (registry + configurable path)`), tag `v0.2-mission087`, GitHub Release publiée. Voir `docs/missions/MISSION_087.md` pour le détail complet.
